...
create table note_snapshots_new (
  ...
  check (
    ...
    or (related_context_type = 'school_date'
      ...
      and related_period_number is null and related_task_item_id is null)
    or (related_context_type = 'daily_lesson'
      and related_school_date is not null
      and related_period_number between 1 and 7
      and related_task_item_id is null)
    or (related_context_type = 'task'
      and related_school_date is null
      and related_period_number is null and related_task_item_id is not null)
  )
);
...
insert into note_snapshots_new select * from note_snapshots;
insert into shared_information_items_new select * from shared_information_items;
...
insert into active_timetable_change_slots_new select * from active_timetable_change_slots;
...
create index note_snapshots_daily_lesson_idx
  on note_snapshots(related_school_date, related_period_number);
create index note_snapshots_task_idx on note_snapshots(related_task_item_id);
create unique index shared_information_changes_direct_source_id_unique
  on shared_information_changes(source_id) where source_type = 'direct';
